...
--두 개의 테이블을 사용해서 조회할 때
--중복된 컬럼이 존재하면 어떤 테이블에서 컬럼을 
--조회할지 결정해야 하는데 그럴 때 주로 테이블에 별칭을 붙여서 
--애매한 컬럼을 조회할 테이블을 지정할 수 있다. 
--별칭을 사용하지 않을 때는 테이블명.컬럼으올 지정할 수도 있다. 
SELECT ST.SNO
     , ST.SNAME
     , ST.SYEAR
    FROM STUDENT ST,
...
--4-2. NVL 사용
--JAVA와 연동했을 때 null값이 JAVA로 그대로 넘어가면
--NullPointException이 발생할 확률이 높아지기 때문에
...
--각각 컬럼에 DISTINCT를 붙여주는 게 아니고 
--두 개의 컬럼이 한 쌍의 Dat set이 돼서 
...
--부서별(DNO)로 정렬하는데 급여가 높은 사람 먼저 나오도록(EMP)
--ENO, ENAME, DNO, SAL
SELECT ENO
     , ENAME
     , DNO 
...
--컬럼의 모든 데이터가 값의 타입으로 변경된 다음에 비교를 하게 된다. 
--데이터가 많아질 경우 모든 데이터에서 현변환이 일어나기 때문에 
--쿼리 속도가 매우 저하된다. 
SELECT *
    FROM EMP
    WHERE SAL >= '3000';
...
--학년이 1학년이면서 이름이 '우'로 끝나는 학생 목록 조회 
SELECT *
    FROM STUDENT
    WHERE SYEAR = 1
     AND SNAME LIKE '%우';
     
--회계업무를 하면서 급여가 3000이상이고 이름이 세 자인 직원 목록 조회 
SELECT *
    FROM EMP
    WHERE JOB = '회계'
     AND SAL >= 3000
...
--평점이 2.0이상이거나 이름이 3자인 학생 중 물리전공인 학생 목록 출력
SELECT *
    FROM STUDENT
    WHERE (AVR >= 2.0
        OR SNAME LIKE '___')
