...
--세션을 kill해서 접속 끊어버리는 쿼리 (DB 락 걸렸을 때)
...
---1-2. 인덱스 
--인덱스 생성
--STUDENT 테이블에 SNAME을 인덱스로 생성(비고유 인덱스)
CREATE INDEX STUDENT_SNAME_IDX
ON STUDENT(SNAME);

--여러 개의 컬럼을 선택하여 복합 인덱스 생성
...
CREATE OR REPLACE VIEW A_AVG_SCORE(
    CNO,
    CNAME,
    MAJOR,
...
--조회 쿼리가 1학년만 가져오는 쿼리라서 원천테이블 추가된 2, 3, 4학년의 데이터는 표출되지 않는다.
...
--CHECK OPTION 추가하면 제약 조건이 생성되어
--조회해 온 조건에 맞는 데이터만 추가할 수 있도록 변경
--1학년 데이터만 조회해오기 때문에 
...
--뷰의 삭제
DROP VIEW ST_CH;
DROP VIEW A_AVG_SCORE;

--인라인 뷰와 LOWNUM 이용해서 최상위 N데이터 조회
--급여 최상위 3명 데이터 조회
SELECT ROWNUM 
     , A.ENO
     , A.ENAME
...
--위치를 잘 지정해야 하고 최상위 SELECT 구문에서의 ROWNUM은 
-- <, <= 비교만 가능하다. 
SELECT ROWNUM
     , A.ENO
     , A.ENAME
...
VALUES (EMP_CO_ENO_SEQ.NEXTVAL, 'G', '개발', NULL, SYSDATE, 3000, 100, 0);

--CURRVAL: 시퀀스가 현재 가리키고 있는 값을 가져온다.
INSERT INTO EMP_COPY1
VALUES (EMP_CO_ENO_SEQ.CURRVAL, 'F', '개발', NULL, SYSDATE, 3000, 100, 0);
...
CREATE SEQUENCE EMP_CO_DNO_SEQ;

INSERT INTO DEPT_COPY1
VALUES(EMP_CO_DNO_SEQ.NEXTVAL, '개발', '서울', 0);
...
--시퀀스의 현재 값을 조회
SELECT EMP_CO_DNO_SEQ.CURRVAL FROM DUAL;

--시퀀스의 다음 값 조회
--NEXTVAL 호출 시 무조건 시퀀스가 다음 값으로 세팅됨. 시퀀스가 가리키는 값이 다음 값으로 넘어감
SELECT EMP_CO_DNO_SEQ.NEXTVAL FROM DUAL;

--시퀀스의 수정
ALTER SEQUENCE EMP_CO_DNO_SEQ
    INCREMENT BY 5
    MAXVALUE 100
    CYCLE;
    
--시퀀스의 삭제 
DROP SEQUENCE EMP_CO_DNO_SEQ;
