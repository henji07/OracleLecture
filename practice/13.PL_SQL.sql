--1. PL/SQL
--1-6. 레코드
--레코드는 다양한 데이터 타입의 변수를 갖는 데이터들의 집합(자바의 클래스에서 메소드만 빠진 형태랑 비슷)
DECLARE
    --레코드 선언부 
...
--레코드와 배열을 조합하여 레코드 타입의 배열 생성 (자바의 객체배열과 비슷)
DECLARE
       TYPE STU_REC IS RECORD (
        --데이터가 저장될 테이블의 컬럼의 순서와 타입을 모두 동일하게 맞춰야 한다. 
        SNO VARCHAR2(5) NOT NULL := '11012',
...
BEGIN 
    ...
    --EXIST 함수는 TRUE/FALSE를 리턴하기 때문에 출력하는 매개변수로 사용 불가
    --IF나 CASE 조건절에 사용한다. 
...
--여러 행을 담고 있는 CURSOR의 FOR LOOP 처리 
DECLARE
    CURSOR CURST IS
        SELECT SNO
             , SNAME
