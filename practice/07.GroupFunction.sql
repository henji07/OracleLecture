...
--마지막에는 모든 그룹화 컬럼에 대한 그룹화가 진행되지 않은 전체 데이터에 대한 결과 조회 
SELECT DNO
     , JOB
     , MAX(SAL)
     , SUM(SAL)
...
SELECT DNO
     ...
     , COUNT(*)
    FROM EMP
    GROUP BY ROLLUP(DNO, JOB, ENO)
    ORDER BY DNO, JOB, ENO;
    
--1-2. CUBE
--CUBE는 ROLLUP과 지정방식이 동일하지만 동작방식이 다르다. 
--그룹화되는 컬럼들의 모든 조합의 그룹화를 진행하여 결과를 출력해주는 함수 
SELECT DNO
     , JOB
     , MAX(SAL)
     , SUM(SAL)
...
--1-3. GROUPING SETS
--그룹화로 지정된 컬럼들의 각각의 그룹화를 진행한 결과를 리턴
SELECT DNO
     , JOB
     , MAX(SAL)
     , SUM(SAL)
...
--PIVOTDML 메인 FROM절은 사용할 컬럼만 가져올 수 있도록 서브쿼리로 테이블 재구성하든가
--사용할 컬럼만 가지고 있는 테이블을 CREATE TABLE로 새로 만들어서 사용
SELECT *
    FROM (
            SELECT MAJOR
                 , SYEAR
