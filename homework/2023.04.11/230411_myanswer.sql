...
SELECT SAL * 1.1 * 12 AS 급여인상
    FROM EMP;

--5) 현재 학생의 평균 평점은 4.0만점이다. 이를 4.5만점으로 환산해서 검색하라(별칭을 사용)
SELECT ST_NUM * 4.5/4.0 AS 환산점수
    FROM COURSE;
    
--1) '__학과인 __학생의 현재 평점은 __입니다' 형태로 학생의 정보를 출력하라
SELECT MAJOR || '과인 ' || SNAME || '학생의 현재 평점은 ' || AVR || '입니다.'
    FROM STUDENT;
...
SELECT PNAME || '교수는 ' || SECTION || '소속입니다.'
    FROM PROFESSOR;
...
--4) 학과별 학년별로 학생의 정보를 성적 높은 순으로 검색하라
SELECT MAJOR
     , SYEAR
     , AVR
    FROM STUDENT 
...
--8) 각 학과별로 교수의 정보를 부임일자 오래된 순으로 검색하라
SELECT *
    FROM PROFESSOR
