...
CREATE TABLE STUDENT_COPY (
    ...
    MAJOR VARCHAR2(20),
    AVR NUMBER(4, 2),
    CONSTRAINT PK_STC_SNO PRIMARY KEY(SNO)
);
...
CREATE TABLE COURSE_CONTSRAINT (
    ...
    PNO VARCHAR2(8),
    CONSTRAINT PK_CR_CNO_CNAME PRIMARY KEY(CNO, CNAME),
    CONSTRAINT FK_CR_PNO FOREIGN KEY(PNO)
...
SELECT *
    FROM COURSE_CONTSRAINT;

--4) 다음 구조를 갖는 테이블을 생성하세요.
--   T_SNS                              T_SNS_DETAIL                        T_SNS_UPLOADED
--   SNS_NO(PK)    SNS_NM VARCHAR2(20)  SNS_NO(PK, FK)   SNS_BEN            SNS_NO(PK, FK)    SNS_UPL_NO(PK)
--     1            페북                   1               4000                   1                  1
--     2           인스타                  2               10000                  1                  2
--     3           트위터                  3               30000                  2                  1
--                                                                               2                  2
...
CREATE TABLE T_SNS_DETAIL (
    SNS_NO NUMBER,
    SNS_BEN NUMBER,
    CONSTRAINT PK_SNS_DETAIL_NO PRIMARY KEY(SNS_NO),
    CONSTRAINT FK_SNS_DETAIL_NO FOREIGN KEY(SNS_NO)
...
CREATE TABLE PAYMENT  (
    ...
    TOTAL NUMBER,
    CONSTRAINT PK_PAYMENT_PNO PRIMARY KEY(MNO),
    CONSTRAINT PAYMENT_TOTAL CHECK(TOTAL > 0)
);
...
CREATE TABLE PAYMENT_DETAIL (
    ...
    CONSTRAINT PK_PAYMENT_DETAIL_MNO PRIMARY KEY(MNO, PNO),
    ...
    CONSTRAINT PAYMENT_DETAIL_TOTAL CHECK(TOTAL_PRICE > 0)
);
